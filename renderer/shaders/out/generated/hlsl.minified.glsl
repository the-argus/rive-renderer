...
#define j0 D1
#define M0 g
typedef min16int c6;typedef min16uint M;
#define O0 min16uint
#define k5 half3x4
#define p inline
#define x2(h4) out h4
...
#define V0 };
#define v0(d6,r,a,G) G a=r.a
#define e8(c) register(b##c)
#define X3(c,a) cbuffer a:e8(c){struct{
#define G4(a) }a;}
#define A1 struct X{
#define k0 noperspective
#define OPTIONALLY_FLAT nointerpolation
#define e6 nointerpolation
...
#define f8(d2,a) SamplerState a:register(s##d2);
#define J3 f8
#define l3 f8
#define I1(a,F) a[F]
...
#endif
#define a1(e,l) e[H]=(l)
p uint g6(B3<uint>h6,m0 H,uint x){uint L1;InterlockedMax(h6[H],x,L1);return L1;}
#define B4(e,S0) g6(e,H,S0)
p uint i6(B3<uint>h6,m0 H,uint x){uint L1;InterlockedAdd(h6[H],x,L1);return L1;}
#define C4(e,S0) i6(e,H,S0)
#define y0(e)
#define D4(w0)
#define U2
#define j3
#define g1(a,P,r,j,L) cbuffer Ya:e8(z7){uint ia;uint a##Za;uint a##ab;uint a##bb;}X a(P r,uint j:SV_VertexID,uint ja:SV_InstanceID){uint L=ja+ia;X K;
#define l5(a,P,r,j,L) X a(P r,uint j:SV_VertexID){X K;g q1;
...
#define z0(a,A0) a[A0]
#define q2(a,A0) a[A0]
p l0 unpackHalf2x16(uint u){uint y=(u>>16);uint x=u&0xffffu;return Z1(f16tof32(x),f16tof32(y));}p uint packHalf2x16(d R0){uint x=f32tof16(R0.x);uint y=f32tof16(R0.y);return(y<<16)|x;}p i unpackUnorm4x8(uint u){T e2=T(u&0xffu,(u>>8)&0xffu,(u>>16)&0xffu,u>>24);return g(e2)*(1./255.);}p uint packUnorm4x8(i f){T e2=(T(f*255.)&0xff)<<T(0,8,16,24);e2.xy|=e2.zw;e2.x|=e2.y;return e2.x;}p float atan(float y,float x){return atan2(y,x);}p A inverse(A e1){A ka=A(e1[1][1],-e1[0][1],-e1[1][0],e1[0][0]);return ka*(1./determinant(e1));}