...
#define U2 ,ma r1,la M1
#define j3 ,r1,M1
#ifdef UC
#define g1(a,P,r,j,L) __attribute__((visibility("default")))X vertex a(uint j[[vertex_id]],uint L[[instance_id]],constant uint&na[[buffer(z7)]],constant RB&v[[buffer(O2)]],constant P*r[[buffer(0)]]U2){L+=na;X K;
#else
...
#endif
#define l5(a,P,r,j,L) __attribute__((visibility("default")))X vertex a(uint j[[vertex_id]],constant RB&v[[buffer(O2)]],constant YB&J[[buffer(w3)]],constant P*r[[buffer(0)]]U2){X K;
...
#define n1
#define o1
#define k4(a) __attribute__((visibility("default")))fragment a(H0 Y,constant RB&v[[buffer(O2)]],X K[[stage_in]],c3 r1,a3 M1){d n0=K.q1.xy;x0 H=x0(metal::floor(n0));uint N1=H.y*v.i7+H.x;
#define g8(a) __attribute__((visibility("default")))fragment a(H0 Y,constant RB&v[[buffer(O2)]],constant YB&J[[buffer(w3)]],X K[[stage_in]],c3 r1,a3 M1){d n0=K.q1.xy;x0 H=x0(metal::floor(n0));uint N1=H.y*v.i7+H.x;
#define R1(a) void k4(a)
#define U3(a) void g8(a)
#define c2 }
#define n3(a) i k4(a){i P0;
#define z4(a) i g8(a){i P0;
...
p uint g6(thread uint&n,uint x){uint L1=n;n=metal::max(L1,x);return L1;}
#define B4(e,S0) g6(Y.e,S0)
p uint i6(thread uint&n,uint x){uint L1=n;n=L1+x;return L1;}
#define C4(e,S0) i6(Y.e,S0)
#define n1
#define o1
...
#define c2 }return Y;
#define h8(a,...) struct oa{i pa[[f(0)]];H0 Y;};oa __attribute__((visibility("default")))fragment a(__VA_ARGS__){d n0[[maybe_unused]]=K.q1.xy;i P0;H0 Y;
#define n3(a) h8(a,H0 f2,X K[[stage_in]],c3 r1,a3 M1)
#define z4(a) h8(a,H0 f2,X K[[stage_in]],c3 r1,a3 M1,__VA_ARGS__ constant YB&J[[buffer(w3)]])
#define V3 }return{.pa=P0,.Y=Y};
#endif
#define discard discard_fragment()
using namespace metal;template<int z1>p vec<uint,z1>floatBitsToUint(vec<float,z1>x){return as_type<vec<uint,z1>>(x);}template<int z1>p vec<int,z1>floatBitsToInt(vec<float,z1>x){return as_type<vec<int,z1>>(x);}p uint floatBitsToUint(float x){return as_type<uint>(x);}p int floatBitsToInt(float x){return as_type<int>(x);}template<int z1>p vec<float,z1>uintBitsToFloat(vec<uint,z1>x){return as_type<vec<float,z1>>(x);}p float uintBitsToFloat(uint x){return as_type<float>(x);}p l0 unpackHalf2x16(uint x){return as_type<l0>(x);}p uint packHalf2x16(l0 x){return as_type<uint>(x);}p i unpackUnorm4x8(uint x){return unpack_unorm4x8_to_half(x);}p uint packUnorm4x8(i x){return pack_half_to_unorm4x8(x);}p A inverse(A e1){A k6=A(e1[1][1],-e1[0][1],-e1[1][0],e1[0][0]);float qa=(k6[0][0]*e1[0][0])+(k6[0][1]*e1[1][0]);return k6*(1/qa);}p O mix(O m,O b,d8 e0){O i8;for(int k=0;k<3;++k)i8[k]=e0[k]?b[k]:m[k];return i8;}p f6 k5(O m,h b,O e0,h ra,O l6,h f0){return f6(m.x,m.y,m.z,b,e0.x,e0.y,e0.z,ra,l6.x,l6.y,l6.z,f0);}
)===";
} // namespace glsl
} // namespace pls
} // namespace rive